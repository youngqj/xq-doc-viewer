...
// Polyfill Node.js `global` for browser — ofd-xml-parser uses global.xmlParseFlag
...
export const parseOfdDocument = function (options) {
    if (options.ofd instanceof File || options.ofd instanceof ArrayBuffer) {
        doParseOFD(options);
    } else if (typeof options.ofd === 'string') {
        ...
        // Blob or other
        if (options.ofd instanceof Blob) {
            options.ofd.arrayBuffer().then(data => {
                options.ofd = data;
                doParseOFD(options);
            }).catch(err => {
                if (options.fail) {
...
const doParseOFD = function (options) {
    pipeline(async () => await unzipOfd(options.ofd), getDocRoots, parseSingleDoc)
        .then(res => {
            if (options.success) {
                options.success(res);
            }
...
export const renderOfd = function (screenWidth, ofd) {
    ...
    for (const page of ofd.pages) {
        ...
        pageDiv.id = pageId;
        pageDiv.setAttribute('style', `margin-bottom: 20px;position: relative;overflow: hidden;width:${box.w}px;height:${box.h}px;background: white;`)
