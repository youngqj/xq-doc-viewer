...
// Module-level state (replaces global.xmlParseFlag and global.toBeChecked)
let _xmlParseFlag = 0;
let _toBeChecked = new Map();

export { _toBeChecked as toBeChecked };
...
const getSignatureData = async function (zip, signature, signatureID) {
    ...
    _toBeChecked.set(signatureID, arr);
    return {
...
const getSealDocumentObj = function (stampAnnot) {
    return new Promise((resolve, reject) => {
        pipeline(async () => await unzipOfd(stampAnnot.sealObj.ofdArray), getDocRoots, parseSingleDoc)
            .then(res => {
                resolve(res)
            })
            .catch(res => {
